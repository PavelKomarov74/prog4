...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	sMs := meanSpeed(action, duration) * kmhInMsec // Средняя Скорость В Метрах В Секунду

	r := math.Pow(sMs, 2) // вычесляем квадрат

	return ((walkingCaloriesWeightMultiplier*weight + (r/(height/cmInM))*walkingSpeedHeightMultiplier*weight) * duration * minInH)
